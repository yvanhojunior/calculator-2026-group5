currencies_codes = {
    ...
    "AZN": { "name": "Azerbaijani Manat", "symbol": null },
    ...
    "KZT": { "name": "Kazakhstani Tenge", "symbol": null },
    ...
    "VES": { "name": "Venezuelan Bolivar", "symbol": "Bs" },
    ...
    "ZWG": { "name": "Zimbabwe Gold", "symbol": "Z$" },
...
async function loadCurrencyList() {
    ...
    try {
        const storedData = localStorage.getItem('currency_list');
        if (storedData) {
            data = JSON.parse(storedData);
            // Check if cached data is still valid
            if (current_time < data.next_update && current_time >= data.last_updated) {
                const byCode = {};
        ...
        console.warn('Failed to load currency list from localStorage:', e);
    }
    ...
    try {
        ...
        const refreshedCurrencies = Array.isArray(refreshData.currencies)
            ...
            : Object.entries(refreshData.conversion_rates || {}).map(([code, rate]) => ({
                  code,
                  name: currencies_codes[code]?.name || code,
                  rate,
                  symbol: currencies_codes[code]?.symbol || ''
              }));
        ...
        throw new Error('Unable to load currency list: ' + e.message);
    }
}
